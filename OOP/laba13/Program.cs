    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                Serialization.JsonSerializion(new Test(name: "Vladimir11", mark: 4, time: 42));
                Serialization.JsonSerializion(new Test(name: "Vladimir12", mark: 4, time: 42));
                Serialization.JsonDeserializion();

                Console.WriteLine("Xml\n\n");
                Serialization.XmlSerializion(new TestXml(name: "Vladimir13", age: 32));
                Serialization.XmlDeserializion();


                Console.WriteLine("Binary\n\n");
                Serialization.BinarySerializion(new Test(name: "Vladimir14", mark: 4, time: 42));
                Serialization.BinaryDeserializion();

                List<Test> tests = new List<Test>();
                tests.Add(new Test(name: "Vladimir15", mark: 4, time: 42));
                tests.Add(new Test(name: "Vladimir16", mark: 4, time: 42));
                tests.Add(new Test(name: "Vladimir17", mark: 4, time: 42));

                List<TestXml> tests1 = new List<TestXml>();
                tests1.Add(new TestXml(name: "Vladimir8", age: 32));
                tests1.Add(new TestXml(name: "Vladimir9", age: 32));
                tests1.Add(new TestXml(name: "Vladimir10", age: 32));
